...
public:
	...
	string data; // Data the node holds
	Node(string val) { // Constructor for a node
...
public:
	...
	void Insert(string val) { // Function for inserting a string into the tree
		if (IsEmpty() == true) {
	...
	string Find(string val) { // Function for finding a string in a binary tree
		if (IsEmpty() == true) {
			...
			if (temp == nullptr) {
				return ""; // Returns a blank string if the desired string wasn't found in the tree
			}
			else {
				return temp->data;
			}
	...
	string Remove(string val) { // Function for removing a desired item from a binary tree
		if (IsEmpty() == true) {
			throw string("The tree is empty.  Nothing can be removed."); // Throws an error if the tree is empty
		}
		else {
			Node* temp = root;
			string retVal; // Will hold the value of the string to be removed and returned from the Remove function
