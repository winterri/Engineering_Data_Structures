...
class Vertex {
public:
	string value; // value in the vertex
	...
	Vertex(string val) { // constructor for new vertex with value
		value = val;
		edge = nullptr;
	}
};
...
public:
	...
	bool HasVertex(string i) { // if there is a vertex of the value passed
	...
	void addVertex(string i) { // add vertex to array
		if (IsFull()) {
			...
			while (data[index].value != "x") { // finds next empty spot
				index = index + 1;
			}
	...
	string RemoveVertex(string i) { // remove vertex from the array
		if (IsEmpty()) {
			cout << "The graph is empty. No vertices can be removed" << endl;
		}
		else {
			if (HasVertex(i)) { // if the vertex exists
				vector<string> inEdges = InEdges(i); // stores in edges
				vector<string> outEdges = OutEdges(i); // stores out edges
				if (inEdges.empty() && outEdges.empty()) { // if there are no edges
					for (int k = 0; k < graphSize; k++) {
						if (data[k].value == i) { // if the array is at the vertex
							string retVal = data[k].value;
							data[k].value = "x"; // remove the vertex
							count = count - 1;
							return retVal;
						}
					}
					...
					string retVal = data[index].value;
					data[index].edge = nullptr; // remove from array
					data[index].value = "x"; // remove from array
					count = count - 1;
					return retVal;
				}
	...
	void addEdge(string i,string j) { // add the edge
	...
	void RemoveEdge(string i, string j) { // removing the edge from the graph
	...
	bool HasEdge(string i, string j) { // if the edge exists in the graph
	...
	vector<string> OutEdges(string i) { // finds the outedges of the value passed
		vector<string> outEdges; // vector to hold the outedge values
	...
	vector<string> InEdges(string i) { // returns list of the in edges
		vector<string> inEdges; // holds inedge values
		if (IsEmpty()) { // if empty
			cout << "The graph is empty" << endl;
			return inEdges;
		}
	...
	void PrintGraph() { // prints the values in the graph
		if (IsEmpty()) {
			...
			for (int i = 0; i < graphSize; i++) { // iterates through the array
				if (data[i].value == "x") { // if the value is null
					cout << "NULL" << endl;
				}
				else {
